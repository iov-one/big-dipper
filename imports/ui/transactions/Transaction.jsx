...
import starnameFee from './starnameFee.js';
    ...
    render(){
        ...
        if (this.props.loading){
            ...
            if (this.props.transactionExist){
                ...
                return <Container id="transaction">
                    <Helmet>
                        <title>Transaction {tx.txhash} On The Starname Network By IOV | The Big Dipper</title>
                        ...
                        <CardBody>
                            <Row>
                                ...
                                <Col md={8} className="value">{(tx.tx.value.fee.amount.length > 0)?tx.tx.value.fee.amount.map((fee,i) => {
                                    return <span className="text-nowrap" key={i}> {new Coin(parseFloat(fee.amount)+starnameFee(tx), fee.denom).stakeString()} </span>
